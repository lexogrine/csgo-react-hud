...
export default class Kill extends React.Component<{
  event: ExtendedKillEvent | BombEvent;
}> {
  render() {
    ...
    if (event.type !== "kill") {
      return (
        <div className={`single_kill`}>
          ...
          <div className="way">
            <img
              src={event.type === "plant" ? C4 : DefuseCT}
              style={{ maxHeight: "18px" }}
              alt={`[${event.type.toUpperCase()}]`}
            />
          </div>
    ...
    return (
      <div className={`single_kill`}>
        {event.attackerblind ? (
          <img src={flash_kill} className="flash_kill" alt={"[FLASHED]"} />
        ) : (
          ""
        )}
        ...
        <div className="way">
          ...
          {event.thrusmoke ? (
            <img src={smoke_kill} className="smoke_kill" alt={"[SMOKE]"} />
          ) : (
            ""
          )}
          {event.noscope ? (
          ...
          {event.wallbang ? (
            <img src={wallbang} className="wallbang" alt={"[WALLBANG]"} />
          ) : (
            ""
          )}
          {event.headshot ? (
